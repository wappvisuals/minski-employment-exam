    ...
    <div class="flex flex-wrap">
        <div class="w-full px-3">
            ...
            <div class="flex flex-1 flex-col justify-center max-full bg-[#232b3a] p-6 rounded-xl">
                <h4 class="text-lg text-gray-100 font-semibold mb-6" data-config-id="auto-txt-40-2">Update Client</h4>
                <div class="flex flex-col space-y-5">
                    <form v-on:submit.prevent="submit($v)" class="space-y-5">
                        ...
                        <button :disabled="isRequesting ? true : false" :class="isRequesting ? 'cursor-not-allowed' : ''" class="btn btn-primary shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 flex item-center justify-center">
                            <span v-if="!isRequesting">Update Client</span>
...
export default {
        ...
        computed: {
            ...mapGetters("client", {
                client: 'GET_CLIENT_DETAILS_GETTER',
            }),
            ...mapGetters("interest", {
        ...
        methods: {
            submit($v) {
                ...
                let params = {
                    'param' : [this.client.id],
                    'payload' : formData
                }

                this._updateClient(params).then(res => {
                    this.$router.push({ name: 'clients' });
                })
            ...
            ...mapActions("client", {
                _updateClient: 'UPDATE_CLIENT_ACTION'
            }),
            ...mapActions("interest", {
